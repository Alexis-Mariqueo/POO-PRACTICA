...
class Carta(ABC):
    ...
    def _calcular_atributos(self,min,max,opcion): ###METODO PROTEGIDO
        if opcion == 1:
        ...
        if opcion == 3:
            self._velocidad = self.__cargar_atributo(min,max)
            self._tiro = self.__cargar_atributo(min,max)
            self._regate = self.__cargar_atributo(min,max)
            self._defensa = self.__cargar_atributo(min,max)
            self._pase = self.__cargar_atributo(min,max)
            self._fisico = self.__cargar_atributo(min,max)
    
    def __cargar_atributo(self,min,max): ##METODO PRIVADO
